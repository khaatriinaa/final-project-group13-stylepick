...
const CLOTHING_SIZES = ['XS', 'S', 'M', 'L', 'XL', 'XXL', '2XL', '3XL', 'One Size'];
const SHOE_SIZES    = ['35–37', '38–40', '41–43', '44–46', '47–49', '50–52'];
    ...
    <View style={styles.body}>
      ...
      <View style={styles.card}>
        <View style={styles.cardHeaderRow}>
          ...
          <Text style={styles.cardSubLabel}>{images.length}/5</Text>
        </View>
        <View style={styles.cardBody}>
          ...
          <Text style={styles.mediaHint}>First image becomes the cover</Text>
        </View>
      </View>

      {/* ── BASIC INFO ──────────────────────────────────── */}
      <View style={styles.card}>
        <View style={styles.cardHeaderSingle}>
        ...
        <View style={styles.cardBody}>
          ...
          <View style={styles.statusRow}>
            <Pressable
              style={isActive ? styles.statusBtnActive : styles.statusBtn}
              onPress={() => setIsActive(true)}
            >
              <View style={[styles.statusDot, { backgroundColor: isActive ? '#22C55E' : '#9CA3AF' }]} />
              <Text style={isActive ? styles.statusTextActive : styles.statusText}>Active</Text>
            </Pressable>
            <Pressable
              style={!isActive ? styles.statusBtnSelected : styles.statusBtn}
              onPress={() => setIsActive(false)}
            >
              <View style={[styles.statusDot, { backgroundColor: '#9CA3AF' }]} />
              <Text style={!isActive ? styles.statusTextSelected : styles.statusText}>Inactive</Text>
            </Pressable>
          </View>
        </View>
      </View>
      ...
      <View style={styles.card}>
        <View style={styles.cardHeaderSingle}>
          <Text style={styles.cardTitle}>Category <Text style={styles.req}>*</Text></Text>
        </View>
        <View style={styles.cardBody}>
          {categories.length > 0 && (
      ...
      <View style={styles.card}>
        <View style={styles.cardHeaderRow}>
          <Text style={[styles.cardTitle, { marginBottom: 0 }]}>Available Sizes</Text>
          {sizes.length > 0 && (
        ...
        <View style={styles.cardBody}>
          ...
          {/* ── Clothing Sizes Container ── */}
          <View style={styles.sizeGroupContainer}>
            <View style={styles.sizeGroupHeader}>
              <Text style={styles.sizeGroupIcon}>👕</Text>
              <Text style={styles.sizeGroupTitle}>Clothing</Text>
              {sizes.filter(s => CLOTHING_SIZES.includes(s)).length > 0 && (
                <View style={styles.sizeGroupBadge}>
                  <Text style={styles.sizeGroupBadgeText}>
                    {sizes.filter(s => CLOTHING_SIZES.includes(s)).length} selected
                  </Text>
                </View>
              )}
            </View>
          ...
          {/* ── Footwear Sizes Container ── */}
          <View style={styles.sizeGroupContainer}>
            <View style={styles.sizeGroupHeader}>
              <Text style={styles.sizeGroupIcon}>👟</Text>
              <Text style={styles.sizeGroupTitle}>Footwear (EU)</Text>
              {sizes.filter(s => SHOE_SIZES.includes(s)).length > 0 && (
                <View style={styles.sizeGroupBadge}>
                  <Text style={styles.sizeGroupBadgeText}>
                    {sizes.filter(s => SHOE_SIZES.includes(s)).length} selected
                  </Text>
                </View>
              )}
            </View>
            <View style={styles.sizeChipWrap}>
              {SHOE_SIZES.map((size) => {
                ...
                return (
                  <Pressable
                    key={size}
                    style={[active ? styles.sizeChipActive : styles.sizeChip, styles.sizeChipWide]}
                    onPress={() => toggleSize(size)}
                  >
                    <Text style={active ? styles.sizeTextActive : styles.sizeText}>{size}</Text>
                  </Pressable>
      ...
      <View style={styles.card}>
        <View style={styles.cardHeaderRow}>
          <Text style={[styles.cardTitle, { marginBottom: 0 }]}>Available Colors</Text>
          {colors.length > 0 && (
            <Text style={styles.cardSubLabel}>{colors.length} selected</Text>
          )}
        </View>
        <View style={styles.cardBody}>
      ...
      <View style={styles.card}>
        <View style={styles.cardHeaderRow}>
          <Text style={[styles.cardTitle, { marginBottom: 0 }]}>Stock</Text>
          <Pressable style={styles.quickSetBtn} onPress={() => setShowStockModal(true)}>
            <Text style={styles.quickSetText}>Quick Set</Text>
          </Pressable>
        </View>
        <View style={styles.cardBody}>
