...
const uploadAvatar = async (localUri: string, userId: string): Promise<string> => {
  ...
  try {
    ...
    console.log('Seller avatar uploaded:', data.publicUrl); // ✅ DEBUG
    return data.publicUrl;
  } catch (err) {
    console.warn('Seller avatar upload failed:', err); // ✅ DEBUG
    return localUri;
  }
};
// ─────────────────────────────────────────────────────────────────────────────
...
export default function SellerProfileScreen({ navigation }: SellerProfileScreenProps) {
  ...
  return (
    <ScrollView style={s.container} showsVerticalScrollIndicator={false}>
      ...
      {/* ── Header ── */}
      <View style={s.header}>
        <Pressable style={s.avatarWrap} onPress={handlePicture} disabled={avatarLoading}>
          {imageUri ? (
